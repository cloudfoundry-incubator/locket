...
var _ = Describe("LockPick", func() {
	...
	Context("RegisterTTL", func() {
		BeforeEach(func() {
			fakeLockDB.CompareAndReleaseReturns(true, nil)
		})
		...
		It("checks that the lock expires after the ttl", func() {
			...
			Eventually(fakeLockDB.CompareAndReleaseCallCount).Should(Equal(1))
			_, oldLock := fakeLockDB.CompareAndReleaseArgsForCall(0)
		...
		Context("when comparing and releasing the lock fails", func() {
			BeforeEach(func() {
				fakeLockDB.CompareAndReleaseReturns(false, errors.New("failed-to-fetch-lock"))
			})
		...
		Context("when there is already a check process running", func() {
			...
			Context("and the lock id is the same", func() {
				Context("and the lock index is incremented", func() {
					...
					It("cancels the existing check and adds a new one", func() {
						...
						Eventually(fakeLockDB.CompareAndReleaseCallCount).Should(Equal(1))
						Consistently(fakeLockDB.CompareAndReleaseCallCount).Should(Equal(1))
						_, lock := fakeLockDB.CompareAndReleaseArgsForCall(0)
				...
				Context("and competes with a newer lock on checking expiry", func() {
					...
					BeforeEach(func() {
						...
						trigger = 1
						fakeLockDB.CompareAndReleaseStub = func(logger lager.Logger, lock *db.Lock) (bool, error) {
					...
					It("checks the expiration of the lock twice", func() {
						...
						Eventually(fakeLockDB.CompareAndReleaseCallCount).Should(Equal(2))
						Consistently(fakeLockDB.CompareAndReleaseCallCount).Should(Equal(2))
					})
				})
				...
				Context("when registering an older lock", func() {
					...
					Context("and the previous lock has already expired", func() {
						...
						It("checks the expiration of the lock", func() {
							...
							Eventually(fakeLockDB.CompareAndReleaseCallCount).Should(Equal(2))
							_, lock := fakeLockDB.CompareAndReleaseArgsForCall(1)
			...
			Context("when another lock is registered", func() {
				...
				It("does not effect the other check goroutines", func() {
					...
					Eventually(fakeLockDB.CompareAndReleaseCallCount).Should(Equal(2))
					_, lock1 := fakeLockDB.CompareAndReleaseArgsForCall(0)
					_, lock2 := fakeLockDB.CompareAndReleaseArgsForCall(1)
			...
			Context("and the check process finishes", func() {
				BeforeEach(func() {
					...
					Eventually(fakeLockDB.CompareAndReleaseCallCount).Should(Equal(1))
					Consistently(fakeLockDB.CompareAndReleaseCallCount).Should(Equal(1))
					_, l := fakeLockDB.CompareAndReleaseArgsForCall(0)
				...
				It("performs the expiration check", func() {
					...
					Eventually(fakeLockDB.CompareAndReleaseCallCount).Should(Equal(2))
					Consistently(fakeLockDB.CompareAndReleaseCallCount).Should(Equal(2))
					_, l := fakeLockDB.CompareAndReleaseArgsForCall(1)
					Expect(l).To(Equal(lock))
				})
			})
		})
