...
type FakeLockDB struct {
	...
	CompareAndReleaseStub        func(logger lager.Logger, lock *db.Lock) (bool, error)
	compareAndReleaseMutex       sync.RWMutex
	compareAndReleaseArgsForCall []struct {
		logger lager.Logger
		lock   *db.Lock
	}
	compareAndReleaseReturns struct {
		result1 bool
		result2 error
	}
	compareAndReleaseReturnsOnCall map[int]struct {
		result1 bool
		result2 error
	}
...
func (fake *FakeLockDB) CompareAndRelease(logger lager.Logger, lock *db.Lock) (bool, error) {
	fake.compareAndReleaseMutex.Lock()
	ret, specificReturn := fake.compareAndReleaseReturnsOnCall[len(fake.compareAndReleaseArgsForCall)]
	fake.compareAndReleaseArgsForCall = append(fake.compareAndReleaseArgsForCall, struct {
		logger lager.Logger
		lock   *db.Lock
	}{logger, lock})
	fake.recordInvocation("CompareAndRelease", []interface{}{logger, lock})
	fake.compareAndReleaseMutex.Unlock()
	if fake.CompareAndReleaseStub != nil {
		return fake.CompareAndReleaseStub(logger, lock)
	}
	if specificReturn {
		return ret.result1, ret.result2
	}
	return fake.compareAndReleaseReturns.result1, fake.compareAndReleaseReturns.result2
}

func (fake *FakeLockDB) CompareAndReleaseCallCount() int {
	fake.compareAndReleaseMutex.RLock()
	defer fake.compareAndReleaseMutex.RUnlock()
	return len(fake.compareAndReleaseArgsForCall)
}

func (fake *FakeLockDB) CompareAndReleaseArgsForCall(i int) (lager.Logger, *db.Lock) {
	fake.compareAndReleaseMutex.RLock()
	defer fake.compareAndReleaseMutex.RUnlock()
	return fake.compareAndReleaseArgsForCall[i].logger, fake.compareAndReleaseArgsForCall[i].lock
}

func (fake *FakeLockDB) CompareAndReleaseReturns(result1 bool, result2 error) {
	fake.CompareAndReleaseStub = nil
	fake.compareAndReleaseReturns = struct {
		result1 bool
		result2 error
	}{result1, result2}
}

func (fake *FakeLockDB) CompareAndReleaseReturnsOnCall(i int, result1 bool, result2 error) {
	fake.CompareAndReleaseStub = nil
	if fake.compareAndReleaseReturnsOnCall == nil {
		fake.compareAndReleaseReturnsOnCall = make(map[int]struct {
			result1 bool
			result2 error
		})
	}
	fake.compareAndReleaseReturnsOnCall[i] = struct {
		result1 bool
		result2 error
	}{result1, result2}
}
